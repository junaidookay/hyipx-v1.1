...
function showAmount($amount, $decimal = 2, $separate = true, $exceptZeros = false, $currencyFormat = true) {
    ...
    if ($currencyFormat) {
        $general = gs();
        if ($general->currency_format == Status::CUR_BOTH) {
            if ($general->cur_sym == $general->cur_text) {
                return $general->cur_sym . $printAmount;
            }
            return $general->cur_sym . $printAmount . ' ' . __($general->cur_text);
        } else if ($general->currency_format == Status::CUR_TEXT) {
            return $printAmount . ' ' . __($general->cur_text);
        } else {
            return $general->cur_sym . $printAmount;
        }
    }
...
function getDefaultLangs($general = null) {
    return;
}
