...
class NotifyProcess {
    ...
    protected function replaceShortCode($name, $username, $template, $body) {
        ...
        $message = str_replace("{{fullname}}", (string)$name, $template);
        $message = str_replace("{{username}}", (string)$username, $message);
        $message = str_replace("{{message}}", (string)$body, $message);
        return $message;
    }
    ...
    protected function replaceTemplateShortCode($content) {
        foreach ($this->shortCodes ?? [] as $code => $value) {
            ...
            $content = str_replace('{{' . $code . '}}', (string)$value, $content);
        }
        return $content;
    }
        ...
        if ($this->template) {
            ...
            if ($this->shortCodes) {
                foreach ($this->shortCodes as $code => $value) {
                    ...
                    $subject = str_replace('{{' . $code . '}}', (string)$value, $subject);
                }
            }
