        ...
        <div class="col-lg-8 col-md-8 mb-30">
            ...
            <div class="card b-radius--10 overflow-hidden box--shadow1">
                <div class="card-body">
                    ...
                    @if (@$withdrawal->user->bank_card)
                        @php
                            $bankCard = ($withdrawal->user->bank_card);
                        @endphp
                        <h6>Account Number or Address</h6>
                        <p>{{@$bankCard->method_address}}</p>
                    @endif


                    @if($details != null)
                        @php $details = json_decode($details); @endphp
                        @if($details && (is_array($details) || is_object($details)))
                                ...
                                <div class="row mt-4">
                                    <div class="col-md-12">
                                        @if(is_object($val))
                                            ...
                                            @endif
                                        @else
                                            {{-- Fallback for simple key-value pairs (AI Manager) --}}
                    ...
                    @if($withdrawal->status == Status::PAYMENT_PENDING)
                        <div class="row mt-4">
                            <div class="col-md-12">
                                <button class="btn btn-outline--success ms-1 approveBtn" data-id="{{ $withdrawal->id }}" data-amount="{{ showAmount($withdrawal->final_amount, currencyFormat: false) }} {{$withdrawal->currency}}">
