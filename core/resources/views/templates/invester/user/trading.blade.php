...
<link rel="stylesheet" href="https://maxst.icons8.com/vue-static/landings/line-awesome/line-awesome/1.3.0/css/line-awesome.min.css">
        ...
        <div class="px-[5px] pb-[8px] mt-4 mb-4">
            <h1 class="text-3xl font-bold text-white font-orbitron">@lang('Live Trading')</h1>
            <p class="text-blue-200/80 text-sm">@lang('Binary Options Market')</p>
        </div>
            ...
            <div class="glass-card rounded-2xl p-4 relative overflow-hidden">
                ...
                <div class="flex items-center justify-between mb-4 relative z-10">
                    <h3 class="text-lg font-bold text-white font-orbitron flex items-center gap-2">
                        <i class="las la-chart-bar text-yellow-400"></i> @lang('Place Order')
                    </h3>
                ...
                <form id="tradeForm" class="space-y-4 relative z-10">
                    <!-- Asset Selection -->
                    <div>
                         <label class="text-xs font-bold text-blue-200 ml-1 mb-1 block uppercase">@lang('Asset')</label>
                         <div class="relative">
                             <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                <i class="las la-coins text-lg text-blue-400"></i>
                             </div>
                             <select name="coin" id="coinSelect" class="w-full bg-black/40 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-white text-sm focus:outline-none focus:border-blue-500 appearance-none font-orbitron">
                                @foreach($cryptos as $symbol => $name)
                                    <option value="{{ $symbol }}" class="bg-black text-white">{{ $name }} ({{ $symbol }})</option>
                                @endforeach
                             </select>
                    ...
                    <div class="grid grid-cols-2 gap-3 mt-2">
                        <button type="button" class="trade-btn relative w-full group overflow-hidden rounded-xl p-[1px] transform active:scale-95 transition-all" data-value="up">
                            ...
                            <div class="relative bg-black/20 hover:bg-transparent backdrop-blur-sm rounded-xl h-full flex flex-col items-center justify-center py-3 transition-all">
                                ...
                                <span class="font-bold text-white text-sm font-orbitron tracking-wider">@lang('CALL')</span>
                                <span class="text-[9px] text-white/80">@lang('High')</span>
                            </div>
                        </button>
                        ...
                        <button type="button" class="trade-btn relative w-full group overflow-hidden rounded-xl p-[1px] transform active:scale-95 transition-all" data-value="down">
                            ...
                            <div class="relative bg-black/20 hover:bg-transparent backdrop-blur-sm rounded-xl h-full flex flex-col items-center justify-center py-3 transition-all">
                                ...
                                <span class="font-bold text-white text-sm font-orbitron tracking-wider">@lang('PUT')</span>
                                <span class="text-[9px] text-white/80">@lang('Low')</span>
                            </div>
                        </button>
                    </div>
                </form>
...
<script>
    (function($){
        ...
        function loadTradeHistory() {
            $.get('{{ route('user.trading.history') }}', function(response) {
                ...
                if(response.html) {
                    ...
                    initCountdowns();
                     ...
                     $('#tradeHistoryBody').html('<tr class="bg-transparent"><td colspan="4" class="px-4 py-6 text-center text-gray-500 text-xs bg-transparent">No recent trades found</td></tr>');
                }
